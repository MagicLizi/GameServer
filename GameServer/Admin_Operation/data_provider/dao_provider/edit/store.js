...
store.addStore =  function(model,callback){
    var sql = new sqlCommand('INSERT INTO edit_Store(Store_Name,Currency_Id,Enable,Description,Create_Date,Refresh_Time,' +
        'Refresh_Expend,Appear_Pre)VALUES(?,?,?,?,?,?,?,?);',[model.Store_Name,model.Currency_Id,model.Enable,
        model.Description,model.Create_Date,model.Refresh_Time,model.Refresh_Expend,model.Appear_Pre]);
...
store.updateStore =  function(model,callback){
    var sql = new sqlCommand('UPDATE edit_Store SET Store_Name = ?,Currency_Id = ?,Enable = ?,Description = ?,Pool_Id = ?' +
        ',Refresh_Time = ?,Refresh_Expend = ?,Appear_Pre = ? WHERE Store_Id = ?;',[model.Store_Name,model.Currency_Id,
        model.Enable,model.Description,model.Pool_Id,model.Refresh_Time,model.Refresh_Expend,model.Appear_Pre,
        model.Store_Id]);
    sqlClient.insert(sql,function(error,result){
        if(error!=null){
            console.log(error);
...
store.addGoods =  function(model,callback){
    var sql = new sqlCommand('INSERT INTO edit_Goods (Item_Id,Item_Amount,Price,Store_Id,Create_Date,Enable,Goods_Order,' +
        'Icon_Url,Start_Date,End_Date,First_Bonus,Task_Order)VALUES(?,?,?,?,?,?,?,?,?,?,?,?);',[model.Item_Id,model.Item_Amount,model.Price,
        model.Store_Id,model.Create_Date,model.Enable,model.Goods_Order,model.Icon_Url,model.Start_Date,model.End_Date,
        model.First_Bonus,model.Task_Order]);
    sqlClient.insert(sql,function(error,result){
        if(error!=null){
            console.log(error);
            callback(code.DB.INSERT_DATA_ERROR,null);
...
store.updateGoods =  function(model,callback){
    ...
    var sql = new sqlCommand('UPDATE edit_Goods SET Item_Id =?,Item_Amount=?,Price= ?,Store_Id=?,Create_Date=?,Enable=? ' +
        ', Goods_Order = ? , Icon_Url = ?,Start_Date = ?,End_Date = ?,First_Bonus = ?,Task_Order = ? WHERE Goods_Id = ?;',[model.Item_Id,model.Item_Amount,
        model.Price,model.Store_Id,model.Create_Date,model.Enable,model.Goods_Order,model.Icon_Url,model.Start_Date,
        model.End_Date,model.First_Bonus,model.Task_Order,model.Goods_Id]);
