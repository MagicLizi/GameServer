...
item.getAllMerger = function (callback) {
    ...
    'm.* FROM admin_'+process.argv[3]+'.Item_Merge as m,admin_DEV.edit_Items as i where m.Source_Item_Id = i.Item_Id) as s,' +
    '(SELECT i.Item_Name as Target_Item_Name,m.* FROM admin_'+process.argv[3]+'.Item_Merge as m,admin_'+process.argv[3]+'.edit_Items as i ' +
...
item.addItem = function (model, callback) {
    var sql = new sqlCommand('INSERT INTO edit_Items(Item_Name,Item_Order,Item_Type,Amount_Limit,Description,' +
        'Item_Quality,Price,Icon_Url,Create_Date,Equip_Level,Auto_Open)VALUES(?,?,?,?,?,?,?,?,?,?,?);',
        [model.Item_Name,model.Item_Order,model.Item_Type,model.Amount_Limit,model.Description,
            model.Item_Quality,model.Price,model.Icon_Url,model.Create_Date,model.Equip_Level,model.Auto_Open]);
...
item.editItem = function (model, callback) {
    ...
    var sql = new sqlCommand('UPDATE edit_Items SET Item_Name = ?,Item_Order = ?,Item_Type = ?,Amount_Limit = ?,' +
        'Description = ?,Item_Quality = ?,Price = ?,Icon_Url = ?,Equip_Level = ?,Auto_Open= ? WHERE Item_Id = ?',
        [model.Item_Name,model.Item_Order,model.Item_Type,model.Amount_Limit,
            ...
            model.Equip_Level,model.Auto_Open,model.Item_Id]);
