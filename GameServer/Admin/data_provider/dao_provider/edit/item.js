...
item.getAll = function (callback) {
    var sql = new sqlCommand('SELECT * FROM edit_Items ORDER BY Item_Order');
...
item.getAllMerger = function (callback) {
    var sql = new sqlCommand('select s.Source_Item_Name,t.Target_Item_Name,s.Merger_Id,s.Source_Item_Amount,' +
        ...
        'm.* FROM admin_DEV.Item_Merge as m,admin_DEV.edit_Items as i where m.Source_Item_Id = i.Item_Id) as s,' +
        '(SELECT i.Item_Name as Target_Item_Name,m.* FROM admin_DEV.Item_Merge as m,admin_DEV.edit_Items as i ' +
...
item.getItemByType = function (item_Type,callback) {
    var sql = new sqlCommand('SELECT * FROM edit_Items WHERE Item_Type=? ORDER BY Item_Order',[item_Type]);
    sqlClient.query(sql, function(error, results){
        if(error)
            callback(new Error(code.DB.INSERT_DATA_ERROR), code.DB.INSERT_DATA_ERROR);
        else{
...
item.delItemEffectById = function (relation_Id,callback) {
    //console.log(merger_Id);
    var sql = new sqlCommand('DELETE FROM Item_Effect_Relation WHERE Relation_Id = ? ',[relation_Id]);
    sqlClient.query(sql, function(error, results){
...
item.delItemMergeById = function (merger_Id,callback) {
    console.log(merger_Id);
    var sql = new sqlCommand('DELETE FROM Item_Merge WHERE Merger_Id = ? ',[merger_Id]);
    sqlClient.query(sql, function(error, results){
        console.log(error,"!!!!!",results);
        if(error)
            callback(new Error(code.DB.DELETE_DATA_ERROR), code.DB.DELETE_DATA_ERROR);
...
item.getItemEffectByType = function (effect_Type,callback) {
    var sql = new sqlCommand('SELECT * FROM Item_Effect_Relation WHERE Effect_Type = ?  ORDER BY Effect_Value DESC',[effect_Type]);
    sqlClient.query(sql, function(error, results){
        if(error)
            callback(new Error(code.DB.INSERT_DATA_ERROR), code.DB.INSERT_DATA_ERROR);
        else{
...
item.addItem = function (model, callback) {
    var sql = new sqlCommand('INSERT INTO edit_Items(Item_Name,Item_Order,Item_Type,Amount_Limit,Description,' +
        'Item_Quality,Price,Icon_Url,Create_Date,Equip_Level,Auto_Open,Yellow_Description)VALUES(?,?,?,?,?,?,?,?,?,?,?,?);',
        [model.Item_Name,model.Item_Order,model.Item_Type,model.Amount_Limit,model.Description,
        model.Item_Quality,model.Price,model.Icon_Url,model.Create_Date,model.Equip_Level,model.Auto_Open,model.Yellow_Description]);
...
item.editItem = function (model, callback) {
    ...
    var sql = new sqlCommand('UPDATE edit_Items SET Item_Name = ?,Item_Order = ?,Item_Type = ?,Amount_Limit = ?,' +
        'Description = ?,Item_Quality = ?,Price = ?,Icon_Url = ?,Equip_Level = ?,Auto_Open= ?,Yellow_Description = ?' +
        ' WHERE Item_Id = ?', [model.Item_Name,model.Item_Order,model.Item_Type,model.Amount_Limit,
        model.Description,model.Item_Quality,model.Price,model.Icon_Url,
        model.Equip_Level,model.Auto_Open,model.Yellow_Description,model.Item_Id]);
